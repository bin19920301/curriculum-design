...
public class WorkService {
    ...
    @Autowired
    private WorkingPositionMapper positionMapper;

    @Autowired
    private WorkingStepMapper stepMapper;
    ...
    public Result getTeacherWorkInfoById(WorkBaseParam param) {
        ...
        for (int i = 0; i <processDTOList.size() ; i++) {
            List<WoekingPositionDTO> workingPositionDTOList = positionMapper.findByProcessId(processDTOList.get(i).getId());
            //查找工步
            for (int j = 0; j < workingPositionDTOList.size() ; j++) {
                List<WorkingStepDTO> workingStepDTOList = stepMapper.findByWorkingPositionId(workingPositionDTOList.get(j).getId());
                //工位中放入工步
